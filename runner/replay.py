...
if __name__ == "__main__":
    ...
    # Perpare agent.
    from flow.core.ppo_rnn import *
############################################################################
############################################################################
    Agent_NUM = N_ROWS * N_COLUMNS
    Reward_num = 1    #0代表多个rewards，1代表1个
    write = 0   #是否将模拟哦结果写入文档。0否，1是
    NAME = '2x2_600_preActorAndCritic_PPO_RNN_SOFT'
    rnn_agent = PPO(s_dim=42*Agent_NUM,a_dim=Agent_NUM+1,name=NAME)
    rnn_agent.restore_params(NAME,380)
############################################################################
############################################################################
    ...
    each_line_path = "collected_data/test/test_rnnAsoft_plot_log.txt"
    test_epoch_path = "collected_data/test/test_rnnAsoft_epoch_log.txt"
    ...
    _state = np.array(state).reshape([-1,42 * Agent_NUM])
    ep_r = 0
    ...
    if write == 1:
        ...
        for i in range(3):
            ...
            _state = np.array(_state).reshape([-1,42 * Agent_NUM])
