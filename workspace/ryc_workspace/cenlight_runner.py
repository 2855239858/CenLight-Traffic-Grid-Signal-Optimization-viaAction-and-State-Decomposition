...
#ppo2
# times = '1_{}TEST_normalRNN'.format(INNER_LENGTH)
#ppo3
times = '2noRand_{}longTEST_RNNandLastAction2allState_preCritic{}_batch40_pnormalTrajction_2actorLay100_sepUpdate'.format(INNER_LENGTH,pretrain)
#ppo4
...
def get_myNetParams():
    myNetParams = NetParams(
            inflows=get_inflow(get_inflowrate()),
            additional_params={
            ...
            for pre_step in range(steps):
                ...
                if Reward_num == 0:
                    ...
                    for k in range(Agent_NUM):
                        pre_ep_r += rewards[k]
                        rnn_agent[k].experience_store(state_temp[k], actions[k], rewards[k])
                else:
                        ep_r += rewards
                        rnn_agent[k].experience_store(_state, _actions, rewards)
        ...
        if ep >= 0:
            ...
            for i in range(3):
                ...
                for step in range(steps):
                    ...
                    _state = np.array(_state).reshape([-1,num_state])
                    _actions = [rnn_agent[k].choose_action(_state[k]) for k in range(Agent_NUM)]

                    next_state, rewards, done, _ = env.step(np.array(_actions).reshape([-1]))
        ...
        for i in range(rnn_train_epi):
            ...
            for step in range(steps):
                ...
                if Reward_num == 0:
                    ...
                    for k in range(Agent_NUM):
                        ep_r += rewards[k]
                        rnn_agent[k].experience_store(state_temp[k], _actions[k], rewards[k])
                else:
                        ep_r += rewards
                        rnn_agent[k].experience_store(_state, _actions, rewards)
            ...
            print(ep_r)
            rnn_agent[0].summarize(ep_r, global_counter, 'reward')
        if ep % 10 == 0:
            for i in range(Agent_NUM):
                rnn_agent[i].save_params(NAME,ep)
