...
class PPO(object):
    ...
    def __init__(self, s_dim=32, a_dim=1, name="meme"):
        ...
        with tf.device('/cpu:0'):
            ...
            pi_prob_temp1 = tf.reshape(pi_prob,[-1,self.a_dim])
            oldpi_prob_temp1 = tf.reshape(oldpi_prob,[-1,self.a_dim])

            pi_prob_temp2 = tf.reduce_mean(pi_prob_temp1,axis = 1)
            oldpi_prob_temp2 = tf.reduce_mean(oldpi_prob_temp1,axis = 1)

            ratio = pi_prob_temp2/(oldpi_prob_temp2 + 1e-8)
            ...
            self.writer = tf.summary.FileWriter("baseline/rnn_ppo_all/" + self.name + "_log/", self.sess.graph)
    ...
    def _build_anet(self, name, trainable):
        ...
        with tf.variable_scope(name):
            ...
            for i in range(self.a_dim):
                ...
                #     bias_initializer = tf.constant_initializer(0.1))              # output based on the last output step      
...
##每一步的reward进行一个discount，让越远的reward影响变小
    ...
    def save_params(self,name,ep):
        save_path = self.saver.save(self.sess,'my_net/rnn_ppo_all/{}_ep{}.ckpt'.format(name,ep))
        print("Save to path:",save_path)
    def restore_params(self,name,ep):
        self.saver.restore(self.sess,'my_net/rnn_ppo_all/{}_ep{}.ckpt'.format(name,ep))
