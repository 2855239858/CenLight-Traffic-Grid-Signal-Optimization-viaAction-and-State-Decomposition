...
class PPO(object):
    ...
    def __init__(self, s_dim=32, a_dim=1, name="meme"):
        ...
        with tf.device('/cpu:0'):
            ...
            surr_temp = tf.reshape(surr, [-1,self.a_dim])
            ratio_temp2 = tf.reshape(ratio_temp1, [-1,self.a_dim])
            adv = tf.reshape(self.tfadv, [-1,self.a_dim])

            surr_update = tf.stack([surr_temp[k] for k in range(self.a_dim)], axis = 1)
            ratio_update = tf.stack([ratio_temp2[k] for k in range(self.a_dim)], axis = 1)
            adv_update = tf.stack([adv[k] for k in range(self.a_dim)], axis = 1)
            self.aloss = [[],[],[],[]]
            self.atrain = [[],[],[],[]]

            for i in range(self.a_dim):
                self.aloss[i] = -tf.reduce_mean(tf.minimum(surr_update[i],tf.clip_by_value(ratio_temp2[i],1. - 0.2, 1. + 0.2) * adv_update[i]))
                self.atrain[i] = tf.train.AdamOptimizer(A_LR).minimize(self.aloss[i])
    ...
    def update(self):
        ...
        a = np.array(self.buffer_a).reshape([-1,4])
        # self.batch_size = len(a)
        self.sess.run(self.update_oldpi_op)
        adv = self.sess.run(self.advantage, {self.tfs: s, self.tfdc_r: r})
        # print(np.array(adv).shape)
        # adv = (adv - adv.mean())/(adv.std()+1e-6)     # sometimes helpful
        actor_loss = self.sess.run(self.aloss, {self.tfs: s, self.tfa: a, self.tfadv: adv})
        self.summarize(np.sum(actor_loss),self.global_counter,'Actor_loss')
    ...
    def _build_anet(self, name, trainable):
        ...
        with tf.variable_scope(name):
            ...
            for i in range(self.a_dim):
                out_temp1 = tf.layers.dense(outputs[:, i, :], 2,tf.nn.softmax,trainable = trainable)              # output based on the last output step      
            ...
            out = tf.stack([out_temp2[0],out_temp2[1],out_temp2[2],out_temp2[3] ], axis=1)
        params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=name)
        return out, params
...
##每一步的reward进行一个discount，让越远的reward影响变小
    def trajction_process(self, s_):
        ...
        v_s_ = self.get_v(_s)
        buffer_r = np.array(self.buffer_r).reshape([-1,4])
        buffer = [[],[],[],[]]
        for r in buffer_r[::-1]:
            for i in range(self.a_dim):
